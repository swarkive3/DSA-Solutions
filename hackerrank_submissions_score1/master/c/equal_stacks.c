...
int equalStacks(int h1_count, int* h1, int h2_count, int* h2, int h3_count, int* h3) {
    // 1. Calculate the initial total height of each stack
    ...
    // Indices to track the top of each stack
    // (since we are conceptually 'popping', we just move the index forward)
    int top1 = 0, top2 = 0, top3 = 0;

    // 2. Loop until all three heights are equal
    while (true) {
        // If all heights are equal, we found the max height
        if (sum1 == sum2 && sum2 == sum3) {
            return sum1;
        }

        // Identify the tallest stack and remove its top element
